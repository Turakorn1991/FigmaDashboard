...
const buildDocs = (rows: MoveRow[]): DocRow[] => {
  ...
  for (const r of rows) {
    ...
    map.set(r.docNo, {
      ...
      companyId: r.companyId, company: r.company, docType: docType(r.docNo),
    });
  }
  return [...map.values()];
};
...
export function Page5Permits() {
  ...
  docs.forEach((d) => {
    if (!cMap[d.companyId]) { const base: any = { id: d.companyId, name: d.company, total: 0 }; TYPE_KEYS.forEach((k) => (base[k] = 0)); cMap[d.companyId] = base; companyOrder.push({ id: d.companyId, name: d.company }); }
  ...
  const groupBarTotal = docs.filter((d) => !hiddenSeries.has(d.docType)).length;
  ...
  const antColumns: TableColumnsType<TableRow> = [
    ...
    { title: "เลขที่หนังสือ", dataIndex: "docNo", key: "docNo", width: 140, ...getColSearchProps("docNo", "เลขที่หนังสือ"), render: (v: string) => <span style={{ fontFamily: "monospace", fontWeight: 600 }}>{v}</span> },
    ...
    { title: "ประเภทหนังสือ", dataIndex: "docType", key: "docType", width: 380,
      filters: TYPE_KEYS.map((t) => ({ text: t, value: t })),
      onFilter: (value, record) => record.docType === value,
      render: (v: string) => <span style={{ fontSize: 13, color: "#111827", lineHeight: 1.4 }}>{TYPE_FULL[v]}</span>,
    },
  ...
  const exportExcel = () => {
    const data = docs.map((d, i) => ({
      "#": i + 1, "เลขที่หนังสือ": d.docNo, "วันที่อนุญาต": d.dateTH, "วันที่หมดอายุ": d.expireTH,
      "ประเภทหนังสือ": TYPE_FULL[d.docType], "ผู้ประกอบการ": d.company,
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    ws["!cols"] = [{ wch: 5 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 70 }, { wch: 40 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "หนังสืออนุญาต");
    XLSX.writeFile(wb, "จำนวนหนังสืออนุญาตให้สั่งขายขนย้าย.xlsx");
  };
        ...
        {companyStatusData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={Math.max(companyStatusData.length * 128, 200)}>
            <BarChart data={companyStatusData} layout="vertical" margin={{ left: 10, right: 60, top: 0, bottom: 0 }} barCategoryGap="18%">
              ...
              {visibleTypes.map((t) => (
                <Bar key={t} dataKey={t} fill={TYPE_COLOR[t]} radius={[0, 4, 4, 0]} maxBarSize={22}
                  fillOpacity={activeSeries === undefined || activeSeries === t ? 1 : 0.25}
