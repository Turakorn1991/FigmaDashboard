...
// กลุ่มหน่วยผู้ซื้อ(ผู้รับปลายทาง) — DDL เหมือนเมนู 1 (9 กลุ่ม)
const BUYER_GROUPS = [
  { id: "1", label: "ทหาร" },
...
// map หน่วยผู้ซื้อ → กลุ่ม (deterministic) ให้ relate + กระจายครบทุกกลุ่ม (เหมือนเมนู 1)
...
// ประเภทการขนย้าย (DDL) + trick map: ประเภทขนย้าย "ขายขนย้าย..." → "ขายและขนย้ายให้บุคคลอื่นนอกมาตรา 7" เท่านั้น
const MOVE_CATEGORIES = ["ขนย้ายให้หน่วยงานตามมาตรา 7", "ขายและขนย้ายให้บุคคลอื่นนอกมาตรา 7", "ขนย้ายเพื่อทดสอบ", "ขนย้ายเพื่อจัดแสดง", "ขนย้ายกลับโรงงาน"];
...
const SALE_TRANSPORTS: string[] = ["ขายขนย้ายในราชอาณาจักร", "ขายขนย้ายนอกราชอาณาจักร"];

type MockRow = RawRow & {
  docNo: string; expireDate: string; transportType: TransportType; moveCategory: string;
...
const enrichRow = (r: RawRow): MockRow => {
  ...
  return {
    ...
    moveCategory: SALE_TRANSPORTS.includes(TRANSPORT_TYPES[r.id % 3]) ? MOVE_CAT_SALE : MOVE_CAT_NONSALE[r.id % MOVE_CAT_NONSALE.length],
...
export function Page2Company() {
  ...
  const [a, setA] = useState({ companies: [] as string[], weaponType: "", unit: "", region: "", provinces: [] as string[], buyers: [] as string[], buyerUnits: [] as string[], weapons: [] as string[], transportTypes: [] as string[], moveCategories: [] as string[] });
  const [searched, setSearched] = useState(false);
  const [tablePage, setTablePage] = useState(1);
  const [tablePageSize, setTablePageSize] = useState(10);

  const handleSearch = () => { setA({ companies: f_companies, weaponType: f_weaponType, unit: f_unit, region: f_region, provinces: f_provinces, buyers: f_buyers, buyerUnits: f_buyerUnits, weapons: f_weapons, transportTypes: f_transportTypes, moveCategories: f_moveCategories }); setSearched(true); setTablePage(1); };
  const handleReset  = () => {
    ...
    setA({ companies: [], weaponType: "", unit: "", region: "", provinces: [], buyers: [], buyerUnits: [], weapons: [], transportTypes: [], moveCategories: [] });
    setSearched(false);
  };
  ...
  return (
    <div style={{ fontFamily: FF }}>
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 1px 3px rgba(15,23,42,0.08)", marginBottom: 16 }}>
        ...
        {/* Row 1: ช่วงวันที่ขนย้าย | ประเภทขนย้าย | ประเภทการขนย้าย */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
            <label style={LBL}>ช่วงวันที่ขนย้าย</label>
