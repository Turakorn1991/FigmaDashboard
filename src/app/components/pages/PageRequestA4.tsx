...
const MOCK: RequestRow[] = Array.from({ length: 10 }, (_, i) => ({
  id: i + 1,
  refNo: `อ.4-2568-${String(i + 1).padStart(4, "0")}`,
  company: ["บริษัท ดีเฟนส์ อินดัสทรี จำกัด", "ห้างหุ้นส่วนจำกัด ไทยอาวุธยุทธภัณฑ์", "บริษัท ยุทธนา อาวุธ จำกัด", "บริษัท ซีเคียวริตี้ อินดัสทรี จำกัด"][i % 4],
  taxId: ["0105556123456", "0105548234567", "0105549678901", "0105562890123"][i % 4],
  submitDate: `${8 + i} มิ.ย. 2568`,
  status: ["pending", "reviewing", "approved", "rejected"][i % 4],
  details: "คำขออนุญาตสั่งหรือนำเข้ามาในราชอาณาจักรซึ่งวัตถุหรืออาวุธ จำนวน 200 หน่วย เพื่อใช้ในการผลิตอาวุธภายในประเทศ",
}));

export function PageRequestA4() {
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [detail, setDetail] = useState<RequestRow | null>(null);
  const filtered = MOCK.filter((r) => (statusFilter === "all" || r.status === statusFilter) && (r.refNo.includes(search) || r.company.includes(search)));

  return (
    <div style={{ fontFamily: FF }}>
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 20, fontWeight: 700, color: "#0E1119" }}>คำขออนุญาต แบบ อ.4</div>
        <div style={{ fontSize: 13, color: "#8B8E95", marginTop: 4 }}>คำขออนุญาตสั่งหรือนำเข้ามาในราชอาณาจักรซึ่งวัตถุหรืออาวุธ</div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 1px 3px rgba(15,23,42,0.08)", overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid #F0F0F0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: "#0E1119" }}>รายการคำขออนุญาต แบบ อ.4</span>
          <span style={{ fontSize: 12, color: "#8B8E95" }}>ทั้งหมด {filtered.length} รายการ</span>
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #F0F0F0" }}>
              {["#", "เลขที่อ้างอิง", "ผู้ประกอบการ", "เลขภาษี", "วันที่ยื่น", "สถานะ", ""].map((h) => (
                <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontSize: 13, fontWeight: 700, color: "rgba(0,0,0,0.88)", background: "#fff" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              return (
                <tr key={r.id} style={{ borderBottom: "1px solid #F9FAFB" }}
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600, background: s.bg, color: s.color }}>{s.label}</span>
                  </td>
                  <td style={{ padding: "12px 14px" }}>
                    <button onClick={() => setDetail(r)} style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid #E5E7EB", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: 620, maxWidth: "90vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #F3F4F6", display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: "#0E1119" }}>รายละเอียดคำขออนุญาต แบบ อ.4</div>
                <div style={{ fontSize: 12, color: "#8B8E95" }}>คำขออนุญาตสั่งหรือนำเข้ามาในราชอาณาจักรซึ่งวัตถุหรืออาวุธ</div>
              </div>
